...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const campaignsQuery = query(collection(db, "campaigns"), where("adminId", "==", currentUser.uid));
        const unsubscribeCampaigns = onSnapshot(campaignsQuery, (snapshot) => {
            ...
            if(campaignsData.length > 0) {
              ...
              const submissionsQuery = query(collection(db, "submissions"), where("campaignId", "in", campaignIds.slice(0,30)));
              
              getDocs(submissionsQuery).then(submissionSnapshot => {
                  const submissionsData = submissionSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Submission));
    ...
    useEffect(() => {
        ...
        filteredSubmissions.forEach(sub => {
            ...
            if (timeFilter === '60m') {
                ...
                // Creates a unique key for each hour, e.g., "Wed 3 PM"
    ...
    const totalParticipants = campaigns.reduce((acc, c) => acc + (c.participantCount || 0), 0);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <MetricCard title="Total Campaigns" value={campaigns.length} description="All created campaigns." />
